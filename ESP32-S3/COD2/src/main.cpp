...
FilteredTarget filt[MAX_TARGETS];      // smoothed coordinates for rendering
int    targetCount = 0;

static constexpr float FILTER_ALPHA = 0.25f;  // exponential moving average smoothing
...
struct Trail {
  ...
  // previous drawn bounding box (for restoring from grid)
  int prevMinX = 0;
  int prevMaxX = 0;
  int prevMinY = 0;
  int prevMaxY = 0;
...
// Restore a rectangular region from gridSprite to clean up old trail and target
void restoreGridRegion(int minX, int maxX, int minY, int maxY) {
  int x = max(0, minX);
  int y = max(0, minY);
  int w = min(SCR_W, maxX + 1) - x;
  int h = min(RAD_H, maxY + 1) - y;
  if (w > 0 && h > 0) {
    gridSprite.pushSprite(x, y, x, y, w, h);
  }
}

// Calculate bounding box of a trail's visual extent
void getTrailBounds(const Trail &trail, int &minX, int &maxX, int &minY, int &maxY) {
  minX = SCR_W;
  maxX = 0;
  minY = RAD_H;
  maxY = 0;
  ...
  if (trail.len == 1) {
    const int idx = (trail.head - 1 + TRAIL_LEN) % TRAIL_LEN;
    const int x = trail.points[idx].x;
    const int y0 = trail.points[idx].y + 22;
    const int y1 = trail.points[idx].y + 7;
    minX = min(x - 6, x);
    maxX = max(x + 6, x);
    minY = min(y1, y0);
    maxY = max(y1, y0);
  ...
  for (int i = 0; i < trail.len; i++) {
    const int idx = (oldest + i) % TRAIL_LEN;
    const Pt &pt = trail.points[idx];
    int radius = 3 + (int)(6.0f * (float)i / (float)max(1, trail.len - 1));  // matches drawTrail
    minX = min(minX, pt.x - radius);
    maxX = max(maxX, pt.x + radius);
    minY = min(minY, pt.y - radius);
    maxY = max(maxY, pt.y + radius);
  }
}

void drawFrame(TFT_eSPI &canvas, uint32_t now) {
  // Draw origin marker directly
  canvas.fillCircle(CX, CY, 5, C_ORIGIN);
  canvas.fillCircle(CX, CY, 2, TFT_WHITE);

  // Restore all previously drawn target regions before drawing anything new.
  // This prevents one target's cleanup pass from erasing a nearby target that
  // was already rendered earlier in the same frame.
  for (int t = 0; t < MAX_TARGETS; t++) {
    if (trails[t].id == -1 || !trails[t].prevValid) continue;
    restoreGridRegion(trails[t].prevMinX, trails[t].prevMaxX,
                      trails[t].prevMinY, trails[t].prevMaxY);
    trails[t].prevValid = false;
  }
  ...
  for (int i = 0; i < targetCount; i++) {
    ...
    float fx = filt[i].x;  // use filtered coordinates
    float fy = filt[i].y;
    ...
    // Calculate full bounding box of what we'll draw (trail + target + labels + arrow)
    int minX = px - 50;
    int maxX = px + 50;
    int minY = py - 50;
    int maxY = py + 30;
    
    // Expand to include trail extent
    int tMinX, tMaxX, tMinY, tMaxY;
    getTrailBounds(*trail, tMinX, tMaxX, tMinY, tMaxY);
    minX = min(minX, tMinX);
    maxX = max(maxX, tMaxX);
    minY = min(minY, tMinY);
    maxY = max(maxY, tMaxY);
    
    // Store this frame's bounding box for next frame's cleanup
    trail->prevMinX = minX;
    trail->prevMaxX = maxX;
    trail->prevMinY = minY;
    trail->prevMaxY = maxY;
    trail->prevValid = true;

    // Draw trail and target directly to TFT
...
void sensorTask(void *pvParameters) {
  for (;;) {
    ...
    vTaskDelay(pdMS_TO_TICKS(15)); //was at 10 set to 15 to reduce CPU load could def change back to 10 or move it too 20 if needed
  }
}
...
void setup() {
  ...
  if (!psramFound()) {
    Serial.println("WARNING: PSRAM not detected. Check that PSRAM is "
                    ...
                    "platformio.ini) - the radar sprites below will fail "
  ...
  // No frameSprite needed — draw directly to TFT for hybrid rendering.
  // This saves 300 KB per frame in PSRAM transfers.

  drawGrid();
  gridSprite.pushSprite(0, 0);  // Display the grid background once
  drawInfo(tft);
  drawSensorStatus(tft, true);
...
static uint32_t lastHudUpdateMs = 0;
static constexpr uint32_t HUD_UPDATE_MS = 250;
...
void loop() {
  ...
  if (!sensorAlive) {
    ...
    if (sensorWasAlive) {
      clearAllTrails();
      // Reset filter state when sensor goes offline
      for (int i = 0; i < MAX_TARGETS; i++) {
        filt[i].init = false;
      }
    }
    ...
    clearAllTrails();
    // Reset filter state when sensor comes back online
    for (int i = 0; i < MAX_TARGETS; i++) {
      filt[i].init = false;
    }
  }
  ...
  // Apply exponential moving average filter to coordinates (matched by target ID)
  for (int i = 0; i < targetCount; i++) {
    int targetId = targets[i].id;
    
    // Find or initialize filter slot for this target ID
    int filtSlot = -1;
    for (int f = 0; f < MAX_TARGETS; f++) {
      if (filt[f].init && filt[f].id == targetId) {
        filtSlot = f;
        break;
      }
    }
    
    // If not found, find an unused slot
    if (filtSlot == -1) {
      for (int f = 0; f < MAX_TARGETS; f++) {
        if (!filt[f].init) {
          filtSlot = f;
          filt[f].id = targetId;
          filt[f].x = (float)targets[i].x;
          filt[f].y = (float)targets[i].y;
          filt[f].init = true;
          break;
        }
      }
    ...
    // Apply filter if we have a valid slot
    if (filtSlot != -1) {
      filt[filtSlot].x += FILTER_ALPHA * ((float)targets[i].x - filt[filtSlot].x);
      filt[filtSlot].y += FILTER_ALPHA * ((float)targets[i].y - filt[filtSlot].y);
      
      // Update targets array with filtered coordinates
      targets[i].x = (int)filt[filtSlot].x;
      targets[i].y = (int)filt[filtSlot].y;
    }
  }
  
  // Clear unused filter slots for targets that no longer exist
  for (int f = 0; f < MAX_TARGETS; f++) {
    if (!filt[f].init) continue;
    bool exists = false;
    for (int i = 0; i < targetCount; i++) {
      if (targets[i].id == filt[f].id) {
        exists = true;
        break;
      }
    }
  ...
  // Clear trails for targets that no longer exist and restore their regions
  for (int t = 0; t < MAX_TARGETS; t++) {
    ...
    if (!targetExists) {
      // Restore the area where this trail was drawn
      if (trails[t].prevValid) {
        restoreGridRegion(trails[t].prevMinX, trails[t].prevMaxX,
                         trails[t].prevMinY, trails[t].prevMaxY);
        trails[t].prevValid = false;
      }
  ...
  // Hybrid rendering: draw targets directly to TFT with targeted region updates
  drawFrame(tft, now);

  if (now - lastHudUpdateMs >= HUD_UPDATE_MS) {
    lastHudUpdateMs = now;
    drawInfo(tft);
    drawSensorStatus(tft, sensorAlive);
  }
